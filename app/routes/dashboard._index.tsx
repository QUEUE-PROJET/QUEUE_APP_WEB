...
export async function loader({ request }: LoaderFunctionArgs) {
  // On autorise l'accès aux ENTREPRISE_AGENT et EMPLOYE
  const { user, token } = await requireAuth(request);

  // Si c'est un employé, on force la redirection vers le dashboard de son entreprise
  if (user.role === "EMPLOYE" && user.entreprise_id) {
    ...
    if (currentEntreprise !== user.entreprise_id) {
      // Redirige l'employé vers le dashboard de son entreprise
