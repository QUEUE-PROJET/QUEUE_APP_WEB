...
function ConfirmationModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" data-aos="fade-in" data-aos-duration="300">
      <div
      ...
      />
      <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6" data-aos="zoom-in" data-aos-duration="400" data-aos-delay="100">
...
export default function Tickets() {
  ...
  useEffect(() => {
    ...
    if (fetcher.data && typeof fetcher.data === 'object' && 'tickets' in fetcher.data) {
      ...
      // Rafraîchir les animations AOS après mise à jour des données
      AOS.refresh();
    }
  }, [fetcher.data]);
        ...
        <div className="flex gap-3" data-aos="fade-left">
          {queueStatus === 'in_progress' && (
            <>
              <button
                type="button"
                className="queue-control-btn bg-yellow-400 hover:bg-yellow-500 text-gray-900 px-4 py-2 rounded-lg font-medium text-sm flex items-center gap-2 transition-all duration-300 hover:scale-105"
              ...
              <actionFetcher.Form method="post">
                ...
                <button
                  type="submit"
                  className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg font-medium text-sm flex items-center gap-2 transition-all duration-300 hover:scale-105"
          ...
          {queueStatus === 'paused' && (
            <>
              <button
                type="button"
                className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg font-medium text-sm flex items-center gap-2 transition-all duration-300 hover:scale-105"
                disabled={!serviceId || !agenceId}
          ...
          {queueStatus === 'not_started' && (
            <actionFetcher.Form method="post" data-aos="zoom-in">
              <input type="hidden" name="service_id" value={serviceId} />
              <input type="hidden" name="agence_id" value={agenceId} />
              <button
                type="submit"
        ...
        <table className="w-full">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredAndSortedTickets.length === 0 ? (
              <tr>
                <td colSpan={6} className="text-center py-4 text-gray-500" data-aos="fade-in">
              ...
              filteredAndSortedTickets.map((ticket: Ticket, index: number) => (
                <tr
                  key={ticket.id}
                  className={`ticket-row transition-all duration-300 ${ticket.status === "EN_COURS" ? "bg-blue-50 font-medium border-l-4 border-blue-500" :
                    ticket.status === "EN_PAUSE" ? "bg-yellow-50 font-medium border-l-4 border-yellow-500" : "hover:bg-gray-50"
                    }`}
                  data-aos="fade-right"
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap capitalize">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium transition-all duration-300 ${ticket.status === "EN_COURS"
                        ? "bg-blue-100 text-blue-800 shadow-lg ticket-status-active"
                        : ticket.status === "EN_PAUSE"
                          ? "bg-yellow-100 text-yellow-800 shadow-lg ticket-status-paused"
                    ...
                    <div className="flex gap-2">
                      {ticket.status === "EN_COURS" && (
                        <button
                          ...
                          className="ticket-action-btn px-3 py-1 bg-green-600 hover:bg-green-700 text-white rounded flex items-center gap-1 transition-all duration-300 hover:scale-105"
                      ...
                      {(ticket.status === "EN_ATTENTE" || ticket.status === "EN_COURS" || ticket.status === "EN_PAUSE") && (
                        <button
                          ...
                          className="ticket-action-btn px-3 py-1 bg-red-600 hover:bg-red-700 text-white rounded flex items-center gap-1 transition-all duration-300 hover:scale-105"
