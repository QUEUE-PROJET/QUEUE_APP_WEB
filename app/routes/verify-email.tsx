...
export default function VerifyEmail() {
  ...
  try {
    ...
    alert("Un nouveau code OTP a été envoyé à votre adresse email. Veuillez vérifier votre boîte de réception.");
    
    // Log pour le débogage
    console.log("Réponse de resendEmailOtp:", response);
  } catch (error) {
    console.error("Erreur lors du renvoi du code OTP:", error);
    alert(error.message || "Erreur lors de l'envoi du nouveau code. Veuillez réessayer.");
            ...
            {isRedirecting ? (
              ...
              <Form className="space-y-6" method="post">
                ...
                <div className="text-center mb-4">
                  <div className={`inline-flex items-center px-4 py-2 rounded-full ${timeLeft > 60 ? 'bg-blue-100 text-blue-800' : 'bg-red-100 text-red-800'}`}>
                ...
                <div>
                  ...
                  <div className="mt-2">
                    <input
                      ...
                      value={otp}
                      onChange={(e) => {
                        const value = e.target.value.replace(/\D/g, ''); // N'autorise que les chiffres
                        setOtp(value);
                      }}
                      required
                      className="appearance-none block w-full px-3 py-3 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-lg tracking-widest text-center"
                ...
                <div>
                  <button
                    ...
                    className={`w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white transition-all duration-200 ${otp.length === 6 ? 'bg-blue-600 hover:bg-blue-700 transform hover:-translate-y-1' : 'bg-gray-400 cursor-not-allowed'} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500`}
                  >
                    Vérifier
                  </button>
                </div>
                ...
                <div className="text-center">
                  ...
                  <button
                    ...
                    className={`mt-2 inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm ${
                      resendDisabled 
